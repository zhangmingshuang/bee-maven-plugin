...
public class WebUtils {
    ...
    public static String jsonBodyPost(String url, byte[] content,
        int connectTimeout, int readTimeout, Map<String, String> headers) throws IOException {
        return doPost(url, CONTENT_TYPE_JSON, content, connectTimeout, readTimeout,
            headers);
    }

    public static String doPost(String url, String contentType,
        byte[] content, int connectTimeout, int readTimeout, Map<String, String> headers)
        throws IOException {
        ...
        HttpURLConnection conn = getConnection(new URL(url),
            METHOD_POST, contentType, headers, connectTimeout, readTimeout);
        elapsed.end(Elapsed.HTTP_CONNECTION);

        try {
            elapsed.start(Elapsed.HTTP_WRITE);
            ...
            elapsed.end(Elapsed.HTTP_WRITE);
    ...
    public static String doGet(String url, Map<String, String> params,
        int connectionTimeOut, int readTimeOut, Map<String, String> headers)
        throws IOException {
        URL query = buildGetUrl(url, buildQuery(params));
        return doGet(query,
            DEFAULT_CONTENT_TYPE, null, connectionTimeOut, readTimeOut, headers);
    }

    public static String jsonBodyGet(String url,
        byte[] content, int connectTimeout, int readTimeout, Map<String, String> headers)
        throws IOException {
        return doGet(new URL(url),
            WebUtils.CONTENT_TYPE_JSON, content, connectTimeout, readTimeout, headers);
    }

    public static String doGet(URL url, String contentType,
        byte[] content, int connectTimeout, int readTimeout, Map<String, String> headers)
        throws IOException {
        Elapsed elapsed = Elapsed.get();
        elapsed.start(Elapsed.HTTP_REQUEST);
        ...
        HttpURLConnection conn = getConnection(
            url, METHOD_GET, contentType, headers, connectTimeout, readTimeout);
        elapsed.end(Elapsed.HTTP_CONNECTION);
    ...
    private static HttpURLConnection getConnection(URL url, String method,
        String contentType, Map<String, String> headers, int connectTimeout, int readTimeout)
        throws IOException {
        ...
        for (Entry<String, String> entry : entries) {
            ...
            if (!isAnyEmpty(name, value)) {
                ...
                try {
                    query.append(name)
                        .append(EQ)
                        .append(URLEncoder.encode(value, CHARSET.name()));
