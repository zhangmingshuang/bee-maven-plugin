...
public class BeeTestFileBuilder {
    ...
    public BeeTestFileBuilder javaFile(String packageName) {
        ...
        if (clazzPackageName.indexOf(Config.POINT) != -1) {
            clazzPackageName = clazzPackageName
                .substring(clazzPackageName.lastIndexOf(Config.POINT) + 1);
        }
        javaFile = JavaFile.builder(
            packageName + ".bean." + clazzPackageName, clazzType)
            .build();
        return this;
    }
    ...
    public static BeeTestFileBuilder builder(CtClass aClass) throws BuildException {
        ...
        for (int i = 0, s = declaredMethods.length; i < s; i++) {
            ...
            if (StringUtils.isEmpty(uri)) {
                MojoContexts.getLogger().debug("empty uri. clazz:"
                    + method.getLongName());
                continue;
            }
    ...
    private static TypeSpec buildMethodArgsClass(CtMethod method,
        String methodArgsClassName)
    ...
    private static MethodSpec buildMethodDelegateMethod(CtMethod method, String uri,
        ...
        return MethodSpec.methodBuilder(method.getName())
            ...
            .addStatement("String uri = $T.ofNullable($T.getLocation()).orElse($S) + $S",
                Optional.class, __BeeEnvironment.class, Config.LOCATION, uri)
            .beginControlFlow("if($T.configuration().location().hasBuilder())",
                BeeApplication.class)
            .addStatement("Class clazz = $N.class",
                method.getDeclaringClass().getName())
            .addStatement("Class[] methodArgs = $N",
            ...
            .addStatement("method = clazz.getDeclaredMethod($S, methodArgs) ",
                method.getName())
            .nextControlFlow("else")
            .addStatement("method = clazz.getDeclaredMethod($S)",
                method.getName())
            //end if
            .endControlFlow()
            .addStatement(
                "uri = $T.configuration().location()"
                    + ".buildLocation(uri, clazz, method)",
                BeeApplication.class)
            //try cache
            ...
            .endControlFlow()
            .endControlFlow()
            .addStatement("return new $N(uri)",
                methodDelegateClassName)
            .build();
    }
